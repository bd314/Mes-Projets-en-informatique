...
if($getTargetState == '1' AND $nb_switch == '1' ) {
	...
	shell_exec('sudo /433/433Utils/RPi_utils/codesend '.$code_switch1_on);
	shell_exec('sudo /433/433Utils/RPi_utils/codesend '.$code_switch2_on);
	echo "SUCCESS";
}elseif ($getTargetState == '0' AND $nb_switch == '2') {
	shell_exec('sudo /433/433Utils/RPi_utils/codesend '.$code_switch1_off);
	shell_exec('sudo /433/433Utils/RPi_utils/codesend '.$code_switch2_off);
	echo "SUCCESS";
}elseif ($getTargetState == '1' AND $nb_switch == '3') {
	shell_exec('sudo /433/433Utils/RPi_utils/codesend '.$code_switch1_on);
	shell_exec('sudo /433/433Utils/RPi_utils/codesend '.$code_switch2_on);
	shell_exec('sudo /433/433Utils/RPi_utils/codesend '.$code_switch3_on);
	echo "SUCCESS";
}elseif($getTargetState == '0' AND  $nb_switch == '3'){
	shell_exec('sudo /433/433Utils/RPi_utils/codesend '.$code_switch1_off);
	shell_exec('sudo /433/433Utils/RPi_utils/codesend '.$code_switch2_off);
	shell_exec('sudo /433/433Utils/RPi_utils/codesend '.$code_switch3_off);
	echo "SUCCESS";
}else {
	die("ERROR");
}
