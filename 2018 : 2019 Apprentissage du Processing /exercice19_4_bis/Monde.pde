...
// la cellules est accessibles si le type de la cellule n'est pas un obstacle ou n'a pas déja été visité
  boolean estAccessible(int i, int j) {
    return estDedans(i, j) && cellules[i][j].type != OBSTACLE && cellules[i][j].type != VISITE;
  }

  boolean contientTresor(int i, int j) {
    return estDedans(i, j) && cellules[i][j].type == TRESOR;
  }void visiter(int i, int j) {
    cellules[i][j].type=VISITE;
  }
